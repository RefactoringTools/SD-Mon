...
handle_call({init_conf, SDMON}, {From, _Tag}, State) ->
  case  get_sdmon_config(SDMON, State) of
	  ...
	  Token = 0,
	  {reply, {ok, {Type,  GName, Trace, Nodes, Token}}, 
...
terminate(_Reason, State) ->
    ...
    lists:foreach(fun(Agent) -> do_stop_agent(Agent) end, State),
    init:stop(),
    ok.
...
do_stop_agent(Agent) ->
    ...
    stop_sdmon(SDMON, Nodes),
    io:format("Agent ~p: stopped~n",[SDMON]),
...
%    log("CMD is: ~p~n",[CMD]),
    ...
    new_agent(VM, [{type,Type},{gname,Prefix},{nodes,Nodes},{trace,Trace}]).
...
stop_sdmon(SDMON, Nodes) ->
    ...
    TimeStamp = format(Y)++format(M)++format(D)++"_"++format(H)++format(P)++format(S),
%    spawn(fun() -> 
		  get_trace_files(SDMON, Nodes, TimeStamp),
		  Host = domain(SDMON),
...
get_trace_files(SDMONNode, Nodes, Postfix) ->
    io:format("~nGetting trace files from Agent ~p ...~n",[SDMONNode]),
...
%	    log("SCP CMD: ~p~n",[CMD]),
    ...
    lists:foreach(fun(Node) ->
			  sdmon_trace:decode_trace_file(SDMONNode, Node) end, 
		  Nodes),
    os:cmd("mv " ++Dir++" "++Dir++"_"++Postfix),
    io:format("... available at ~p~n",[Dir++"_"++Postfix]).
