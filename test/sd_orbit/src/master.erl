...
orbit(Gs, Xs,P, TimeOut, SpawnImgComp, Sub_masters) ->
    Credit=credit:one(),
    StartTime = now(), % start wall clock timer
    Group_Hash_Table= start_submasters(Sub_masters, {[], 0}, {Gs, Xs, P, TimeOut, SpawnImgComp,Credit}), %% start submaster nodes and returns Group Hash Table
    ...
    lists:foreach(fun({_,Pid,_,_}) -> Pid ! {start, Group_Hash_Table} end, Group_Hash_Table), %% send Group_Hash_Table to all gateway processes
    ...
    lists:foreach(fun({_,Pid, _, _}) -> Pid ! {dump} end, Group_Hash_Table), %% after collecting all the credits, terminates all gateway processes
...
start_submasters([], { Group_Hash_Table, _GlobalSize}, {_Gs, _Xs, _P, _Timeout, _Spawn, _Credit}) ->
  Group_Hash_Table;

start_submasters([{Node, GroupName, TabSize}], { Group_Hash_Table, GlobalSize}, {Gs, Xs, P, Timeout, Spawn, Credit}) ->
  ...
  [{Pid, Gateway,GlobalSize, TabSize+1} | Group_Hash_Table];
  
start_submasters([{Node, GroupName, TabSize} | Sub_masters], { Group_Hash_Table, GlobalSize}, {Gs, Xs, P, Timeout, Spawn, Credit}) ->
  Pid = spawn_link(Node, sub_master, init,[Gs, Xs, P, Timeout, Spawn, Credit, self(), GroupName]),
  Gateway = spawn_link(Node, sub_master, gateway,[]),
  New_Group_Hash_Table = [{Pid, Gateway,GlobalSize, TabSize} | Group_Hash_Table],
  ...
  start_submasters(Sub_masters, {New_Group_Hash_Table,New_GlobalSize}, {Gs, New_Xs, P, Timeout, Spawn, New_Credit}).
