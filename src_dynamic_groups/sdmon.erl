...
start([MasterNodeStr]) ->
    ...
    {Type, GName, Trace, Nodes, Token} = get_config_from_master(MasterNode),
    log("Received configuration: ~p~n",[{Type, GName, Trace, Nodes, Token}]),
...
get_config_from_master(MasterNode) ->
    case  catch sdmon_master:init_conf(MasterNode) of
    	{ok, {Type, GName, Trace, Nodes, Token}} ->
	    {Type, GName, Trace, Nodes, Token};
...
trace(Node, start, Trace, State) -> 
	...
	VMrec ->
		...
		true ->
		    case sdmon_trace:trace(Node, Trace, VMrec, 
					   get_group_type(State),
					   get_group_name(State)) of
...
build_state({MasterNode, Type, GName, Trace, Nodes, Token})  ->
    #state{master=MasterNode, type=Type, gname=GName, trace=Trace, 
	   nodes=build_nodes(Type, GName, Trace, Nodes, []), token=Token};
build_state(MasterNode) ->
    #state{master=MasterNode}.

%y
build_nodes(_, _, _, [], Recs) ->
    Recs;
build_nodes(GType, GName, Trace, [Node | Nodes], Recs) ->
	...
	false ->
	    ...
	    build_nodes(GType, GName, Trace, Nodes, Recs++[VMrec]);
	_  ->                              % Double entry: discard it
	    build_nodes(GType, GName, Trace, Nodes, Recs)
    end.
	...
	Oldrec ->    
		...
		_ ->
		    log("NODE ~p: Crashed worker ~p with Reason: ~p~n",
		        [Node, Pid, Reason]),
		    case sdmon_trace:trace(Node, {TraceType, Args}, Oldrec, na, na) of
